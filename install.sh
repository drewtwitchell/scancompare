...
install_trivy_manual() {
  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    echo "❌ Unsupported OS for Trivy installation"; return 1
  fi
...
install_grype_manual() {
  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    echo "❌ Unsupported OS for Grype installation"; return 1
  fi
...
# ✅ Enforce correct shebang explicitly as first line
echo "🔧 Enforcing Python shebang..."
tmp_file=$(mktemp)
echo '#!/usr/bin/env python3' > "$tmp_file"
tail -n +2 "$INSTALL_PATH" >> "$tmp_file"
